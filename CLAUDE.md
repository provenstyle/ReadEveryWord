...
### Completing a reading cycle

`Mark Complete` is offered only at 1189 of 1189 chapters — the menu row is disabled below that and says how many are left. Completing sets `dateCompleted`, which makes the grid read-only; `Reopen` clears it again.

`readingCycle.update` is what does both, so `UpdateReadingCycle.dateCompleted` is `string | null | undefined` and **all three states are load-bearing**: absent leaves the stored value alone, a date sets it, and `null` clears it. Hence `{type: ['string', 'null'], format: 'date-time'}` in the schema (`format` only constrains strings) and a `!== undefined` test in the persistence rather than the truthy check that was there before. Clearing also switches `updateEntity` to **`Replace`** mode — Merge only writes the properties it is handed and physically cannot remove one. Replace drops anything the entity does not carry, which is safe only because the row is read in full first; `readingCycle.test.ts` covers the round trip, including that `name`/`dateStarted`/`default` survive it.

Read-only is enforced in the **UI only**, deliberately. `BibleProvider.readOnly` is keyed on `loadedCycleId` (the grid actually painted, not the active cycle) and both `readChapter` and `unreadChapter` return early on it, so `Mark All As Read` cannot slip past the disabled buttons. The api still accepts writes to a completed cycle: guarding it server-side would mean a `getEntity` on the cycle before every chapter write, doubling a toggle from one storage round trip to two and adding 150 to a Psalms sweep, to defend against something no client does. The threat being handled is a stray click, not a hostile caller.
