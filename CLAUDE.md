...
Required runtime env vars: `TABLE_STORAGE_CONNECTION_STRING`, `OPEN_ID_JWKS_URI`, `OPEN_ID_AUDIENCE`, `OPEN_ID_ISSUER`. Integration tests additionally need `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET`, and optionally `AUTH0_DOMAIN` / `AUTH0_AUDIENCE`; the m2m token is cached both in memory and on disk under `packages/api-integration-test/.cache/` because Auth0 requires token reuse.
